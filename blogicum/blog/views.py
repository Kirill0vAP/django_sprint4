...
def profile(request, username):
    ...
    context = {'profile': user, 'page_obj': page_obj, 'is_owner': is_owner}
...
@login_required
def comment_adding(request, post_id):
    post = get_object_or_404(Post, id=post_id)
    if request.method == 'POST':
...
@login_required
def comment_editing(request, post_id, comment_id):
    post = get_object_or_404(Post, id=post_id)
...
@login_required
def comment_deleting(request, post_id, comment_id):
    post = get_object_or_404(Post, id=post_id)
    comment = get_object_or_404(Comment, id=comment_id, post=post)
    if comment.author != request.user:
        return redirect('blog:post_detail', post_id=post.id)
...
@login_required
def post_deleting(request, post_id):
    post = get_object_or_404(Post, id=post_id)
    if post.author != request.user:
        return redirect('blog:post_detail', post_id=post.id)
    if request.method == 'POST':
